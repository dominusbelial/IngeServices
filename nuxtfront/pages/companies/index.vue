      ...
      >
        ...
        >
                    ...
                    <v-layout row justify-center wrap>
                        <v-btn color='green' small :to='`/companies/new`'><v-icon x-small>mdi-playlist-plus</v-icon> Nuevo</v-btn>
...
asyncData({ error, app }) {
    const count = app.$axios.$get('/companies/count')
    return {
        pagination: {
            totalItems: count,
            totalPages: 0,
            ...
            filterByField: 'id',
            listOfFields: ['deleted', 'id', 'legal_representative', 'name', 'created_at', 'updated_at']
        }
    }
},
mounted() {
...
methods: {
    ...
    async getPagedData() {
            const response = await this.$axios.$post('/graphql', {
                query: `query {
                    companies(start: ${(this.pagination.page - 1) * this.pagination.rowsPerPage},
