      ...
      >
        ...
        >
                    ...
                    <v-layout row justify-center wrap>
                        <v-btn color='green' small :to='`/students_companies/new`'><v-icon x-small>mdi-playlist-plus</v-icon> Nuevo</v-btn>
...
asyncData({ error, app }) {
    const count = app.$axios.$get('/students-companies/count')
    return {
        pagination: {
            totalItems: count,
            totalPages: 0,
            ...
            filterByField: 'id',
            listOfFields: ['deleted', 'id', 'name', 'created_at', 'updated_at']
        }
    }
},
mounted() {
...
methods: {
    goToEdit(id){
        this.$router.push('/students_companies/' + id + '/edit')
    },    
    async getPagedData() {
            const response = await this.$axios.$post('/graphql', {
                query: `query {
                    studentsCompanies(start: ${(this.pagination.page - 1) * this.pagination.rowsPerPage},
            ...
            this.loading = false
            this.items = response.data.studentsCompanies
