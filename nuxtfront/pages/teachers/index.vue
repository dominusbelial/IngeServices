    ...
    >
      ...
      >
        ...
        >
                    ...
                    <v-subheader>Teachers</v-subheader>
                    <v-layout row wrap>
                    <v-flex xs4>
                        <v-text-field
                        dense
                    ...
                    <v-layout row justify-center wrap>
                        <v-btn color='green' small :to='`/teachers/new`'><v-icon x-small>mdi-playlist-plus</v-icon> Nuevo</v-btn>
...
asyncData({ error, app }) {
    const count = app.$axios.$get('/teachers/count')
    return {
        pagination: {
            totalItems: count,
            totalPages: 0,
            ...
            filterByField: 'id',
            listOfFields: ['cedula', 'deleted', 'id', 'name', 'position', 'created_at', 'updated_at']
        }
    }
},
mounted() {
...
methods: {
    goToEdit(id){
        this.$router.push('/teachers/' + id + '/edit')
    },    
    async getPagedData() {
            const response = await this.$axios.$post('/graphql', {
                query: `query {
                    teachers(start: ${(this.pagination.page - 1) * this.pagination.rowsPerPage},
            ...
            this.loading = false
            this.items = response.data.teachers
