      ...
      >
        ...
        >
                    ...
                    <v-layout row justify-center wrap>
                        <v-btn color='green' small :to='`/courses/new`'><v-icon x-small>mdi-playlist-plus</v-icon> Nuevo</v-btn>
...
asyncData({ error, app }) {
    const count = app.$axios.$get('/courses/count')
    return {
        pagination: {
            totalItems: count,
            totalPages: 0,
            ...
            filterByField: 'id',
            listOfFields: ['deleted', 'duration', 'id', 'legal_representative', 'name', 'teacher', 'created_at', 'updated_at']
        }
    }
},
mounted() {
...
methods: {
    ...
    async getPagedData() {
            const response = await this.$axios.$post('/graphql', {
                query: `query {
                    courses(start: ${(this.pagination.page - 1) * this.pagination.rowsPerPage},
